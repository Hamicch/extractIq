...
async function seed() {
  ...
  const insertedTenants = await db.insert(tenants).values(testTenants).returning();
  ...
  const insertedApiKeys = await db.insert(apiKeys).values(testApiKeys).returning();
  ...
  const documentTypes = [
    ...
    { type: 'agreement', mime: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', extensions: ['.docx'] },
  ];

  const statuses = ['uploaded', 'processing', 'completed', 'failed'] as const;
  const uploaders = ['john@example.com', 'sarah@company.com', 'admin@startup.io', 'system'];
  ...
  for (let i = 0; i < 50; i++) {
    ...
    const extension = docType.extensions[Math.floor(Math.random() * docType.extensions.length)];
    ...
    sampleDocuments.push({
      ...
      pageCount: status === 'completed' || status === 'processing' ? Math.floor(Math.random() * 20) + 1 : null,
      uploadedBy: uploaders[Math.floor(Math.random() * uploaders.length)],
      createdAt: createdDate,
      processedAt: status === 'completed' ? new Date(createdDate.getTime() + Math.random() * 3600000) : null,
    });
  }
  ...
  const completedDocs = insertedDocuments.filter(doc => doc.status === 'completed');
  const sampleExtractions: NewDocumentExtraction[] = completedDocs.map((doc, idx) => {
    ...
    switch (extractionType) {
      case 'invoice':
        extractedData = {
          ...
          lineItems: [
            { description: 'Professional Services', amount: (Math.random() * 1000).toFixed(2) },
            { description: 'Software License', amount: (Math.random() * 500).toFixed(2) },
          ],
        };
        break;
      case 'contract':
      ...
      case 'receipt':
        extractedData = {
          ...
          items: [
            { name: 'Item 1', price: (Math.random() * 50).toFixed(2) },
          ],
        };
        break;
      default:
    ...
    return {
      ...
      modelVersion: ['gpt-4-vision-preview', 'gpt-4-turbo', 'claude-3-opus'][idx % 3],
      extractedAt: doc.processedAt || new Date(),
    };
  });

  if (sampleExtractions.length > 0) {
    const insertedExtractions = await db.insert(documentExtractions).values(sampleExtractions).returning();
    console.log(`✅ Created ${insertedExtractions.length} document extractions`);
  }
  ...
  for (const doc of insertedDocuments) {
    const stages: Array<'upload' | 'ocr' | 'extract' | 'validate' | 'complete'> = ['upload'];
    ...
    for (let i = 0; i < stages.length; i++) {
      ...
      auditLogs.push({
        ...
        errorMessage: isFailed ? 'OCR processing failed: Invalid image format' : null,
        createdAt: new Date(doc.createdAt.getTime() + i * 1000),
      });
    }
  }

  const insertedAuditLogs = await db.insert(processingAuditLog).values(auditLogs).returning();
